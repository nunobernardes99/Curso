...
class ED098 {
    ...
    private static void lerDoentes(Scanner in) {	
        while (in.hasNext()) {
                ...
                int colorIndex = indiceCor(cor);
                numDoentes++;
                emEspera[colorIndex].fila.enqueue(new Doente(nome, chegada, atendimento));
    ...
    private static void mostrarAtendidos() {
        ...
        for (Doente d : atendidos) { 
            ...
            sum += (d.entrada-d.chegada);
        }
        System.out.println("---------------------------");
        float avg = sum / atendidos.size();
    ...
    private static int proximoDoente(int tempo) {
        for(int i=0; i<emEspera.length; i++)
