...
class Vol4B {
	static int coise(Queue<Arco> t, int origem, int destino, int[] pai, Grafo grafo) {
		int min = Integer.MAX_VALUE;
		int cur = destino;
		System.out.println(Arrays.toString(pai));
		while(t.peek() != null) {
			//System.out.println(min);
			if(grafo.find_arc(pai[cur],cur) == null)
				return 0;
			int temp = grafo.find_arc(pai[cur], cur).valor_arco();
			if(temp < min)
				min = temp;
			if(pai[cur] == origem)
				return min;
	...
	static void Dijkstra(Grafo grafo, int origem, int destino) {
		...
		Queue<Arco> t = new LinkedList<Arco>(); 

		while(!q.isEmpty()) {
			int v = q.extractMin();
			if(grafo.find_arc(pai[v], v) != null)
				t.add(grafo.find_arc(pai[v], v));
			for(Arco adj: grafo.adjs_no(v)) {
				int w = adj.extremo_final();
				if((-1 * adj.valor_arco()) < dist[w] && !ok[w]) {
					dist[w] = -1 * adj.valor_arco();
					pai[w] = v;
					ok[w] = true;
		...
		System.out.println(coise(t, origem, destino, pai, grafo));
	}
	...
	public static void main(String[] args) {
		...
		while(fV != -1) {
			...
			if((lMin <= l) && (cMin <= c) && (aMin <= a)){
				grafo.insert_new_arc(fV, sV, c);
				grafo.insert_new_arc(sV, fV, c);
			}
			fV = scan.nextInt();
		}
