...
void trata_primeiro(int passo, Caixa *caixa_atual){
		...
		Cliente *cliente=getFirstQueueP(caixa_atual->lista);
...
void simulador(int afluencia,int apetencia, int n_caixas, int n_ciclos){
	...
	for(int i=1; i<=n_ciclos; i++){
		...
		if(aux<=(double)(afluencia/100.0)){
			...
			char prioridade='N';
			char *status;
			//Decidir prioridade do cliente criado(Urgente/Normal)
			int temp=rand()%2; 
			if(temp==0){
				status="Normal";
			}
			...
			printf("--> Criado cliente com  %d produtos e com prioridade %s \n", produtos, status);
			printf("\n");
			...
			enqueueP(novo_cliente, caixas[caixa_escolhida]->lista, prioridade);
		}
